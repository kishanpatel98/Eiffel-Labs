...
feature -- model
	model: FUN [K, V]
		...
		do
			...
			across
				implementation.as_array as cursor
			loop
				--model.extend (cursor.item)
				Result.extend ([cursor.item.key, cursor.item.value])
			end
		end
...
feature -- commands
	...
	put (val: V; key: K) --(key: K; val: V)
		...
		do
			if implementation.has (key) then
				replace (key, val)
			else
				extend (key, val)
			end
	...
	prune (val: V; i: INTEGER)
			...
			-- demanding version
		do
			implementation.remove (occurrences (val)[i])
		end
	...
	prune_all (val: V)
		...
		do
			...
			loop
				if cursor.item.value ~ val then
					implementation.remove (cursor.item.key)
				end
			end
		end
	...
	merge (other: SORTED_MAP_ADT[K,V])
		...
		do
			implementation.extend_from_array (other.as_array)
		end
...
feature -- queries
	...
	sorted_keys: ARRAY [K]
		...
		do
			Result := (create {UTIL[K]}).mergesort (model.domain.as_array)
		end
	...
	occurrences (val: V): ARRAY [K]
		...
		do
			Result := (create{UTIL[K]}).mergesort(((model @> val).domain).twin)
		end
